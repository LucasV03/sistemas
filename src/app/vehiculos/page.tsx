...
export default function VehiculosPage() {
  ...
  const registrarMantenimiento = useMutation(
    api.vehiculos.registrarMantenimiento
  );
  ...
  function calcNextService(v: any) {
    ...
    const lastKm = Number.isFinite(v?.ultimoServiceKm)
      ? Number(v.ultimoServiceKm)
      : 0;
  ...
  async function handleAddVehiculo(e: React.FormEvent) {
    ...
    try {
      console.log("Enviando a Convex:", payload);
      ...
      alert(
        `Error al crear vehículo: ${
          err?.message ?? "Ver consola de Convex"
        }`
      );
    }
  }
  ...
  async function handleRegistrarMantenimiento(v: any) {
    try {
      ...
      console.error(err);
      alert(
        `No se pudo registrar mantenimiento: ${
      ...
      <section className="rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4 md:p-6 shadow-sm">
            ...
            <table className="w-full table-fixed">
              <thead className="bg-slate-800 text-white">
                <tr className="text-left select-none">
                  <Th onClick={() => handleSort("patente")}>
                    Patente{arrow("patente")}
                  </Th>
                  <Th onClick={() => handleSort("tipo")}>
                    Tipo{arrow("tipo")}
                  </Th>
                  <Th onClick={() => handleSort("capacidad")}>
                      ...
                      >
                        ...
                        <Td>
                          <div className="flex flex-col">
                            <span
                              className={`inline-flex items-center whitespace-nowrap leading-none w-fit rounded-full px-2 py-0.5 text-xs ring-1 ${badge}`}
                            >
        ...
        <div className="mt-8 rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-4">
          ...
          <form onSubmit={handleAddVehiculo} className="flex flex-wrap gap-3">
            <Field
              label="Patente"
              className="flex-1 basis-full sm:basis-[calc(50%-0.375rem)] min-w-[220px]"
            >
              <input
                type="text"
...
function Td({
  ...
  return (
    <td
      className={[
        "px-3 py-2 h-12 border border-neutral-200 dark:border-neutral-800 text-neutral-800 dark:text-neutral-200",
        className,
      ].join(" ")}
    >
      {children}
